...
class OctaveConv(nn.Module):
    def __init__(self,in_channels,out_channels,kernel_size,alpha_in=0.5,alpha_out=0.5,stride=1,
    padding=0,dilation=1,groups=1,bias=False):
        ...
        self.conv_l2h=None if alpha_in==0 or alpha_out==1 or self.is_dw else\
            nn.Conv2d(int(alpha_in*in_channels),int(out_channels-alpha_out*out_channels),kernel_size,stride,padding,dilation,groups,bias)       
        ...
        self.conv_h2h=None if alpha_in==1 or alpha_out==1 else\
            ...
            kernel_size,stride,padding,dilation,math.ceil(groups-alpha_in*groups),bias)
    ...
    def forward(self,x):
        ...
        x_h2l=self.conv_h2l(x_h) if self.alphaout>0 and not self.is_dw else None
        if x_l is not None:
...
class Conv_BN_ACT(nn.Module):
    ...
    groups=1,bias=False,norm_layer=nn.BatchNorm2d,activation_layer=nn.ReLU):
        ...
        self.conv=OctaveConv(in_channels,out_channels,kernel_size,alpha_in,alpha_out,stride,padding,dilation,groups,bias)
        self.bn_h=None if alpha_out==1 else norm_layer(int(out_channels*(1-alpha_out)))
        self.bn_l=None if alpha_out==0 else norm_layer(int(alpha_out*out_channels))
